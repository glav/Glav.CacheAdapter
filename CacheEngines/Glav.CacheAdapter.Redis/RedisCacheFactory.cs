...
namespace Glav.CacheAdapter.Redis
{
    public class RedisCacheFactory : CacheConstructionFactoryBase
    {
        ...
        public override CacheFactoryComponentResult CreateCacheComponents()
        {
            ...
            if (CacheConfiguration.DependencyManagerToUse == CacheDependencyManagerTypeConfigValues.Generic)
            {
                dependencyMgr = new GenericDependencyManager(cacheEngine, Logger, CacheConfiguration);
            }
            else if (CacheConfiguration.DependencyManagerToUse == RedisDependencyManagerTypeConfigValues.Redis)
            {
                dependencyMgr = new RedisDependencyManager(cacheEngine, Logger, _redisConnection.GetDatabase(), CacheConfiguration);
            } else
            {
                throw new NotSupportedException($"Dependency manager of type: {CacheConfiguration.DependencyManagerToUse} not supported.");
            }
